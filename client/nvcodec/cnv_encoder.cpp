...
int32_t cnv_encoder::InitEncode(const VideoCodec* inst,
  ...
  for (int i = 0, idx = number_of_streams - 1; i < number_of_streams; ++i, --idx) 
  {
    ...
    encoders_[i] = new NvEncoderCuda  (m_cuContext, configurations_[i].width, configurations_[i].height,NV_ENC_BUFFER_FORMAT_IYUV /*NV_ENC_BUFFER_FORMAT_ARGB*/ /* NV_ENC_BUFFER_FORMAT_IYUV*/);
    NORMAL_EX_LOG("");
    NV_ENC_INITIALIZE_PARAMS initializeParams = { NV_ENC_INITIALIZE_PARAMS_VER };
    NV_ENC_CONFIG encodeConfig = { NV_ENC_CONFIG_VER };
    initializeParams.encodeConfig = &encodeConfig;
