...
#define NVENC_THROW_ERROR(errorStr, errorCode)                  \
  do {                                                          \
	using namespace chen;	ERROR_EX_LOG("[%s][%s][%d][errorstr = %s][errorcode = %u]", __FUNCTION__, __FILE__, __LINE__, errorStr, errorCode);				\
...
using namespace chen;	ERROR_EX_LOG("[%s][%s][%d][errorstr = %s][errorcode = %u]", __FUNCTION__, __FILE__, __LINE__, errorLog.str(), errorCode);				\
      throw NVENCException::makeNVENCException(                         \
...
class NvEncoder {
 ...
 protected:
  ...
  bool IsHWEncoderInitialized() const {
	  ...
	  NORMAL_EX_LOG("m_bEncoderInitialized = %u", m_bEncoderInitialized);
